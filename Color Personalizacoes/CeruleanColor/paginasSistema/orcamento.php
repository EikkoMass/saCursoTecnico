...
<!--Encontrar forma de aplicar isso depois da validação JS, como um pop up-->
...
<div id="conteudo">
  ...
  <form method="post" action="../php/registraOrcamento.php" id="formPedido" onsubmit="return validaOrcamento(cont)">
...
<input type="checkbox" name="alternaEntrega" id="alternaEntrega" style="margin-left: 3%; margin-top: 0.5%;" onClick="return entregaEntrega()"><label for="alternaEntrega" style="font-size: 14px;">Local Alternativo</label>
  ...
  <input type="text" name="desconto" id="desconto" placeholder="Desconto %" class="textbox porcento" maxlength="3">
  </div>
...
?></td>
  <td><?=$cadaParaGeral['preco_total']?></td>
     ...
     <td class="tdborder">
        <form action="../php/editaOrcamentoForm.php" method="POST" class="formato editaform">
        <input type="hidden" name="id" value="<?=$cadaParaGeral['id'];?>">
        ...
        <form action="../php/excluiOrcamento.php" method="POST" class="formato" class="apagaForm">
        <input type="hidden" name="id" value="<?=$cadaParaGeral['id'];?>">
